...
{
    INSTALL_HOOK(symbols.vm.arm_vm_init, arm_vm_init_hooked, &arm_vm_hook);
    INSTALL_HOOK(symbols.vm.arm_vm_prot_init, arm_vm_prot_init_hooked, &arm_vm_prot_hook);
    INSTALL_HOOK(symbols.serial.serial_init, serial_init_hooked, &serial_hook);
    INSTALL_HOOK(symbols.kernel_debug_early, kernel_debug_early_hooked, &kernel_debug_hook);
}
...
void arm_vm_prot_init_hooked(boot_args *args)
{
    struct mach_o_segment_command *segment =
        mach_o_get_segment(symbols.mh_execute_header, "__RALLY");

    ((void (*)(boot_args *))(&arm_vm_prot_hook)->original)(args);
    symbols.vm.arm_vm_page_granular_prot(segment->vm_addr, segment->vm_size, 0, 0, AP_RWNA, 0,
...
void kernel_debug_early_hooked(uint32_t debugid, uintptr_t arg1, uintptr_t arg2, uintptr_t arg3,
                               uintptr_t arg4)
{
    if (*symbols.startup_phase >= STARTUP_SUB_KPRINTF) {
        const char *fmt = "%x: %s\n";
        asm volatile("stp x29, x30, [sp, #-16]!\n"
                     ...
                     "ldp x29, x30, [sp], #16\n"
                     :
                     : "r"(arg1), "r"(arg2), "r"(arg3), "r"(arg4), "r"((uint64_t)debugid), "r"(fmt),
                       "r"(symbols.printf)
                     : "x9", "memory");
    }
}